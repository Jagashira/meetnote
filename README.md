...
MeetNoteは、Zoomなどのオンライン会議向けに作られた、ローカルファーストの議事録アプリケーションです。
...
保存先は`MEETNOTE_DATA_DIR`で変更できます。ディレクトリは自動的に作成されます。
